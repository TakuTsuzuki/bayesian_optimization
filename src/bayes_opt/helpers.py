...
class UtilityFunction(object):
    ...
    def __init__(self, kind, kappa, xi):
        ...
        if kind not in ['ucb', 'ei', 'poi','mi']:
            err = "The utility function " \
                  "{} has not been implemented, " \
                  "please choose one of ucb, ei, poi or mi.".format(kind)
            raise NotImplementedError(err)
        else:
            self.kind = kind
        
        # if kind == 'mi' create list that have history of var (std**2)
        if kind == 'mi':
            self.hist_var = np.array([])
    ...
    def _mi(self, x, gp ):
        mean, std = gp.predict(x, return_std=True)
        phi = np.sqrt(std**2 + np.sum(self.hist_var)) - np.sqrt(np.sum(self.hist_var))
        self.hist_var = np.append(self.hist_var, [std**2])
        return mean + self.kappa * phi
